...
// Resize everything on quick view (when an obstruction appears/disappears).
static void quickViewLayerReposition() {
  ...
  // Resize the shifting and center layers based on the current unobstructed
  // bounds.
...
static void main_window_load(Window *window) {
  ...
  // Background layer — draw your watchface canvas here (see drawUtils_*.c).
  ...
  // Info layout — time text plus the configurable widget slots, drawn on top.
...
static void init() {
  APP_LOG(APP_LOG_LEVEL_INFO, "Watchface template init");
